...
%% Current
plot(fixed_step_time, fixed_step_data(:, 6), "LineWidth",2.5), hold on;
plot(fixed_step_time, fixed_step_data(:, 4), "LineWidth",2.5,  "LineStyle",":", "Color", '#fa7a2a'), hold off;
xlim([0 5*(1/60)]);
ylim([-25 25]);
set(gcf,'Position',[100 100 1000 500]);
legend('I_{Source}', 'I_{Load}', fontsize=12);
...
print(gcf,'-djpeg','-r900', 'imgs/[FULL WAVE - Controled] 30deg Currents.jpeg');

%% Measurement Values
measurements_labels(1) = "Mean Source Voltage";
measurements_values{1} = mean(fixed_step_data(:, 1));
measurements_labels(2) = 'RMS Source Voltage';
measurements_values{2} = rms(fixed_step_data(:, 1));

measurements_labels(3) = 'Mean Load Voltage';
measurements_values{3} = mean(fixed_step_data(:, 2));
measurements_labels(4) = 'RMS Load Voltage';
measurements_values{4} = rms(fixed_step_data(:, 2));

measurements_labels(5) = 'Mean Source Current';
measurements_values{5} = mean(fixed_step_data(:, 6));
measurements_labels(6) = 'RMS Source Current';
measurements_values{6} = rms(fixed_step_data(:, 6));

measurements_labels(7) = 'Mean Load Current';
measurements_values{7} = mean(fixed_step_data(:, 4));
measurements_labels(8) = 'RMS Load Current';
measurements_values{8} = rms(fixed_step_data(:, 4));
...
%% %%%%%%%%%%%%%%%%%%%%% Full Bridge - 60 Deg Single Phase %%%%%%%%%%%%%%%%%%%%% %%
% Since the simulation was done on a variable step size, it is necessary to
% create a linear interpolation of fixed step size so all the calculations
% are correct.
load("data/full_wave_controled_60deg.mat");
full_wave_60_deg = ans;

fixed_step_time = min(full_wave_60_deg.Time):0.00001:max(full_wave_60_deg.Time);
original_time = full_wave_60_deg.Time;
data = full_wave_60_deg.Data;
...
print(gcf,'-djpeg','-r900', 'imgs/[FULL WAVE - Controled] 60deg Voltages.jpeg');
...
print(gcf,'-djpeg','-r900', 'imgs/[FULL WAVE - Controled] 60deg Currents.jpeg');
